...
class Ec2Inventory(object):
    ...
    def do_api_calls_update_cache(self):
        ...
        if self.route53_enabled:
            sys.stderr.write("Route53...")
            sys.stderr.flush()
            self.get_route53_records()
            sys.stderr.write("done.  ")
            sys.stderr.flush()

        sys.stderr.write("ec2 instances in each region...")
        sys.stderr.flush()
        for region in self.regions:
            sys.stderr.write("{},".format(region))
...
# Run the script
pool = BotoWorkerPool(workers=3, start_with="get_instances_by_region")
Ec2Inventory()
